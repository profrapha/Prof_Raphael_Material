...
\usepackage[top=7.5 cm, headheight=7.5cm, bottom=1.3cm, footskip=0.5cm, left=1cm, right=1cm]{geometry}
...
\fancyhead[C]{\includegraphics[width=\textwidth]{MATEMATICA_9ANO.png}}
...
\usepackage{titlesec}
\titleformat{\section}{\color{CorTitUm}\fontsize{17}{17}\bfseries}{\thesection}{0em}{}
...
\begin{document}
\raggedcolumns
\section*{UNIDADE 6 - FUNÇÃO QUADRÁTICA (PARTE 1)}
\begin{multicols*}{2}
...
O projeto arquitetônico de uma praça prevê a construção de um espelho d'água retangular envolto por uma calçada de concreto. As dimensões do espelho d'água dependem de uma variável \textbf{x} (medida em metros), conforme ilustrado no esquema da planta baixa.
...
\fill[drop shadow={opacity=0.2, shadow xshift=3pt, shadow yshift=-3pt}, concreto, rounded corners=4pt] (0,0) rectangle (6,4);
...
I) O valor numérico da função para $x = -2$ é igual a \textbf{-17}. \\[0.3cm]
II) Se $x = 0$, a função atinge exatamente o valor do seu coeficiente independente. \\[0.3cm]
III) O coeficiente quadrático dessa função indica que, ao dobrarmos o valor de \textbf{x}, o resultado final também dobrará.

\vspace{0cm} \noindent\textcolor{CorLinha}{\rule{\linewidth}{0.5pt}} \vspace{-0.2cm}
\subsubsection*{QUESTÃO 05}

Um engenheiro civil projeta o pilar central de uma estrutura metálica e define que a capacidade de carga $C$, em toneladas, em função da espessura \textbf{x} do pilar, em centímetros, é regida por uma função do tipo $C(x) = ax^2 + c$. Ele sabe que, se a espessura for nula, a carga suportada é zero e que, para um pilar de \textbf{10 cm} de espessura, a capacidade é de \textbf{150 toneladas}.
...
Considerando as propriedades fundamentais dessa função, julgue os itens a seguir com \textbf{V} para verdadeiro ou \textbf{F} para falso.

A) Se a fábrica não produzir nenhum lote, o prejuízo será de \textbf{300 mil reais}. \\[0.3cm]
B) O coeficiente \textbf{a} negativo indica uma limitação física ou de mercado, sugerindo que o aumento infinito da produção não gerará aumento infinito de lucro. \\[0.3cm]
C) A produção de exatamente \textbf{10 lotes} garante lucro positivo para a empresa.

\vspace{0cm} \noindent\textcolor{CorLinha}{\rule{\linewidth}{0.5pt}} \vspace{-0.2cm}
\subsubsection*{QUESTÃO 07}

Em uma montadora de veículos, uma peça plana de aço precisa ser recortada por uma máquina a laser. O modelo da peça tem formato retangular, e, para aliviar peso, um retângulo central menor será removido, conforme a vista frontal abaixo. O recorte interno e as margens da peça dependem do parâmetro \textbf{x}, medido em milímetros.
...
\fill[drop shadow={opacity=0.3, shadow xshift=2pt, shadow yshift=-2pt}, aco, rounded corners=3pt] (0,0) rectangle (6,5);
...
(ENEM - Adaptada) Um foguete de sinalização marítima é lançado de um navio durante um treinamento de resgate. A altura \textbf{h}, em metros, atingida pelo foguete em relação ao nível do mar, é descrita em função do tempo \textbf{t}, em segundos, por meio da equação termodinâmica simplificada $h(t) = 10 + 40t - 5t^2$.
...
A câmera de alta velocidade registrou três posições chave num plano cartesiano, sendo \textbf{x} e \textbf{f(x)} em metros. As posições são: $(0, 20)$, $(1, 18)$ e $(-1, 18)$. 
...
I) A concavidade desta parábola é voltada para baixo, pois possui um coeficiente linear negativo. \\[0.3cm]
II) O gráfico desta função intercepta o eixo vertical exatamente na origem do sistema cartesiano, no ponto $(0, 0)$. \\[0.3cm]
...
\fill[drop shadow={opacity=0.3, shadow xshift=2pt, shadow yshift=-2pt}, pista] (-1,-0.5) rectangle (7,0);
...
Os pontos onde a parábola corta o eixo horizontal são conhecidos como raízes ou zeros da função, locais onde a altura (o valor de $y$) zera.
...
Ao lançar uma bola de basquete, a trajetória descrita no espaço bidimensional obedece à equação $y = -2x^2 + 12x + 14$, onde \textbf{y} é a altura e \textbf{x} é a distância horizontal percorrida, ambas em metros.
...
I) A bola foi lançada do chão, pois o eixo \textbf{x} representa o solo e a função corta a origem. \\[0.3cm]
II) A distância horizontal percorrida pela bola desde a origem até o momento em que ela toca o solo novamente é de \textbf{7 metros}. \\[0.3cm]
...
\begin{axis}[
    ...
    background/.style={fill=fundo, drop shadow={opacity=0.1}}
]
% Foguete A
...
Sem calcular o vértice, mas utilizando apenas a intersecção com o eixo \textbf{x} (as raízes), demonstre algebricamente qual dos dois foguetes virtuais intercepta o solo (eixo x) nos pontos mais distantes entre si. Calcule a distância entre os pontos de contato para ambas as funções.

\vspace{0cm} \noindent\textcolor{CorLinha}{\rule{\linewidth}{0.5pt}} \vspace{-0.2cm}
\subsubsection*{QUESTÃO 21}

Considere a família de funções quadráticas dadas por $y = ax^2 - 3x + c$. Sabe-se que o gráfico dessa função cruza o eixo vertical no ponto $(0, -10)$ e cruza o eixo horizontal em $x = 5$ e em um outro valor desconhecido.
...
Para não danificar as plantas, um alarme dispara se a temperatura cair abaixo de $0^\circ \text{C}$. Sabendo que o modelo é válido para $0 \le h \le 24$:

A) A temperatura inicial na estufa à meia-noite era igual a \textbf{0$^\circ$ C}. \\[0.3cm]
B) O alarme disparará às \textbf{15 horas} (3 da tarde), pois as raízes indicam os momentos em que a temperatura atinge zero. \\[0.3cm]
C) A parábola tem concavidade voltada para cima, indicando que a temperatura subirá infinitamente. \\[0.3cm]
D) A estufa só atingirá temperatura zero em $h = 15$. \\[0.3cm]
...
I) O eixo de simetria desta parábola é a reta definida pela equação $x = -2$. \\[0.3cm]
II) A função atinge seu valor mínimo exatamente no ponto de ordenada $y = -18$. \\[0.3cm]
...
As coordenadas do vértice podem ajudar a desvendar os coeficientes ocultos de uma função. Sabe-se que a parábola descrita por $g(x) = -3x^2 + bx + 12$ possui o eixo de simetria passando exatamente sobre a reta $x = 2$.
...
Em um torneio colegial de basquete, um arremesso perfeito descreve uma trajetória descrita pela função $h(x) = -0.2x^2 + 1.2x + 1.8$, onde \textbf{h} é a altura da bola em metros e \textbf{x} é a distância horizontal percorrida a partir do momento em que a bola deixa a mão do jogador.
...
\fill[parede, rounded corners=2pt] (0,0) rectangle (8,5);
\fill[quadra, drop shadow={opacity=0.4, shadow xshift=0pt, shadow yshift=-2pt}] (0,0) rectangle (8,0.5);
...
\fill[grama, rounded corners=2pt] (-1,-1) rectangle (7,4);
...
Sabendo que o arco do refletor é moldado pela função algébrica $y = \mfrac{1}{5}x^2$, onde \textbf{x} e \textbf{y} estão em centímetros, determine a profundidade exata (altura máxima \textbf{y}) desse refletor em seu ponto central.
...
I) Se aumentarmos o valor do termo independente \textbf{c} de 10 para 20, a coordenada $x_v$ não sofrerá alteração, mas a parábola inteira se moverá para cima. \\[0.3cm]
II) A coordenada $x_v$ da função $P(x)$ está localizada do lado esquerdo do eixo vertical (possui valor negativo). \\[0.3cm]
III) A menor altura que o gráfico dessa função atinge no eixo \textbf{y} é \textbf{1}.
...
\fill[ceu, rounded corners=2pt] (0,0) rectangle (8,5);
...
O faturamento bruto diário $F$, em reais, é obtido pela multiplicação do preço da passagem pela quantidade de passageiros. Se o aumento for chamado de variável \textbf{x}, a função que rege o faturamento é $F(x) = (50 + x) \cdot (200 - 2x)$.
...
I) Se $\Delta < 0$, a equação não possui solução, o que geometricamente significa que a parábola não intercepta o eixo \textbf{x}. \\[0.3cm]
II) O sinal de $\pm$ presente na Fórmula de Bhaskara é o responsável por garantir que, quando $\Delta > 0$, a parábola cruze o eixo horizontal em dois pontos simétricos em relação ao vértice. \\[0.3cm]
...
Um teste de balística militar avalia a eficiência de um morteiro posicionado no topo de uma colina. O projétil é disparado e a sua altura \textbf{h}, em metros, em relação ao solo plano do vale, em função do tempo \textbf{t}, em segundos, obedece à lei física $h(t) = -5t^2 + 10t + 175$.
...
\draw[thick, <->, white, >=stealth] (-2, 0.2) -- (6, 0.2) node[midway, fill=rio, inner sep=2pt] {\textbf{Vão Livre}};
...
A óptica estuda a fabricação de lentes espelhadas que seguem padrões geométricos exatos. Um laser de testes é disparado perfeitamente na horizontal ao longo do eixo das abscissas (reta $y = 0$). O perfil de uma lente convergente defeituosa foi modelado pela função $f(x) = x^2 - 7x + 10$.
