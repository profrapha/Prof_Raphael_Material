...
\setlength{\espacoPreQuestao}{0.2cm} % Espaço entre a linha fina divisória e a questão

\newlength{\espacoQuestao}
\setlength{\espacoQuestao}{0.1cm}    % Espaço de "respiro" no FINAL de cada questão
...
\begin{minipage}[t]{0.90\linewidth} 
    ...
    \begin{tcolorbox}[nobeforeafter, height=0.83cm, valign=center, colback=white, colframe=CinzaEscuro, arc=6pt, sharp corners=northwest, boxrule=0.8pt, left=10pt, right=10pt]
        \small\bfseries\textcolor{CinzaMedio}{Ano/Série:} \textcolor{Cinzablack}{7º ANO} \hspace{0.2cm} \textcolor{CinzaMedio}{Bloco:} \textcolor{Cinzablack}{1}
    \end{tcolorbox}
\end{minipage}%
...
$$ \textbf{M} = \mfrac{ \left( \mfrac{3}{7} \right)^{4} \cdot \left( \mfrac{3}{7} \right)^{5} }{ \left( \mfrac{3}{7} \right)^{2} } \Rightarrow \text{Marcelo fez:} \quad \mfrac{3}{7}^{(4 \cdot 5)} \div \mfrac{3}{7}^{2} = \mfrac{3}{7}^{20 - 2} = \left( \mfrac{3}{7} \right)^{18} $$
\begin{boxresponda}
